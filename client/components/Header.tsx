  ...
  return (
    <header className="fixed top-0 left-64 right-0 h-24 bg-white z-40 px-12 flex items-center justify-between">
      ...
      <div className="flex items-center gap-6">
        ...
        >
          {isDark ? (
            <Moon className="w-6 h-6 text-gray-4 rotate-90" />
          ) : (
            <Sun className="w-6 h-6 text-gray-1" />
          )}
        </button>

        <button className="relative w-14 h-14 rounded-full bg-gray-200/60 flex items-center justify-center hover:bg-gray-200 transition-colors">
          <Bell className="w-5 h-5 text-gray-1" />
          <div className="absolute top-2 right-2 w-3.5 h-3.5 bg-red-600 rounded-full flex items-center justify-center">
            <span className="text-white text-[6px] font-bold">5</span>
          </div>
        </button>

        <div className="flex items-center gap-3">
          <div className="w-12 h-12 rounded-full border border-gray-3 overflow-hidden">
            <img
